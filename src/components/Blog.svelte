	...
	<div class="mx-auto max-w-7xl">
		...
		{:else}
			<div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
				{#each blogs as blog}
					<div
						class="blog-card transform rounded-xl bg-gradient-to-br from-gray-100 to-gray-50 p-8 shadow-sm transition-transform duration-300 hover:scale-[1.02] hover:shadow-lg"
					>
						<h3 class="mb-4 text-xl font-medium text-black">{blog.title}</h3>
						<p class="mb-6 text-gray-500">{blog.content}</p>
